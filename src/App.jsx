...
// ───────────────────────────────────────────────
// 100% LOCAL — all data saves to this browser. No database, no internet needed.
// ───────────────────────────────────────────────
...
// ── Storage helpers
function db(){
  try { return JSON.parse(localStorage.getItem("pnl_db")) || { accounts: [], data: {} }; }
  catch { return { accounts: [], data: {} }; }
}
function saveDb(d){ try { localStorage.setItem("pnl_db", JSON.stringify(d)); } catch {} }
...
function EquityCurve({ trades, year, month }){
  ...
  const points=useMemo(()=>{
    ...
    for(let d=1;d<=total;d++){ const k=dk(year,month,d); if(trades[k]) run+=trades[k].totalPnl||0; pts.push({d,v:run}); }
    return pts;
  },[trades,year,month]);
  const W=580,H=70,vals=points.map(p=>p.v);
  const min=Math.min(...vals),max=Math.max(...vals),range=max-min||1;
  const toX=d=>(d/total)*W, toY=v=>H-6-((v-min)/range)*(H-12);
  ...
  useEffect(()=>{
    ...
    requestAnimationFrame(()=>requestAnimationFrame(()=>{ el.style.transition="stroke-dashoffset 1s ease"; el.style.strokeDashoffset=0; }));
  },[poly]);
  return(
...
function DayDetail({ accountId, dayKey, initialRows, onClose, onSave }){
  const [rows,setRows]=useState(()=> initialRows.length ? initialRows.map(r=>({...r})) : [newRow()]);
  const [expandedImg,setExpandedImg]=useState(null);
  const [pasteTarget,setPasteTarget]=useState(null);
  const fileRefs=useRef({});

  function newRow(){ return {id:uid(),pair:"",bias:"bullish",rr:"",outcome:"win",profit:"",image_url:"",trade_note:""}; }
  function updateRow(id,f,v){ setRows(p=>p.map(r=>r.id===id?{...r,[f]:v}:r)); }
  function handleImage(id,file){ if(!file) return; const rd=new FileReader(); rd.onload=e=>updateRow(id,"image_url",e.target.result); rd.readAsDataURL(file); }
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.93)",zIndex:200,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"16px 12px",overflowY:"auto"}}>
      ...
      <div style={{background:"#0f0f0f",border:"0.5px solid #222",borderRadius:12,width:"100%",maxWidth:860,padding:18}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
          <div>
            <div style={{fontSize:9,color:"#444",letterSpacing:"0.12em",textTransform:"uppercase"}}>Daily breakdown</div>
                ...
                <tr key={r.id}>
                  ...
                  <td style={{padding:"0 3px 3px"}}>
                    <select value={r.bias} onChange={e=>updateRow(r.id,"bias",e.target.value)} style={{...iSt,width:90,color:r.bias==="bullish"?"#00ff88":"#ff4444"}}>
                      <option value="bullish">▲ Bull</option><option value="bearish">▼ Bear</option>
                    </select>
                  </td>
                  ...
                  <td style={{padding:"0 3px 3px"}}>
                    <div style={{display:"flex",gap:3}}>
                      {["win","loss"].map(o=>(
                        <button key={o} onClick={()=>updateRow(r.id,"outcome",o)} style={{padding:"5px 7px",borderRadius:4,border:"none",cursor:"pointer",fontFamily:"inherit",fontSize:10,fontWeight:700,background:r.outcome===o?(o==="win"?"rgba(0,255,136,0.2)":"rgba(255,68,68,0.2)"):"#1a1a1a",color:r.outcome===o?(o==="win"?"#00ff88":"#ff4444"):"#333"}}>{o.toUpperCase()}</button>
                      ))}
                    </div>
                  </td>
        ...
        <div style={{display:"flex",gap:8,marginTop:12}}>
          ...
          <button onClick={save} style={{background:"#00ff88",color:"#000",border:"none",borderRadius:6,padding:"8px 20px",cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:700}}>SAVE DAY</button>
        </div>
      </div>
    </div>
  );
...
export default function App(){
  ...
  const [activeId,setActiveId]=useState(()=> db().accounts[0]?.id || null);
  ...
  useEffect(()=>{ saveDb(store); },[store]);

  const accounts=store.accounts;
  const activeAcc=accounts.find(a=>a.id===activeId);

  // The active account's data: { trades: {dayKey: {rows:[...]}} }
  const accData=store.data[activeId] || { days: {} };

  // Derive calendar trades map from rows
  const trades=useMemo(()=>{
    const map={};
    const days=accData.days||{};
    Object.keys(days).forEach(k=>{
      const rows=days[k]||[];
      if(!rows.length) return;
      const totalPnl=rows.reduce((s,r)=>s+(parseFloat(r.profit)||0),0);
      const wins=rows.filter(r=>r.outcome==="win").length;
      map[k]={totalPnl,tradeCount:rows.length,wins,losses:rows.length-wins};
    });
    return map;
  },[accData]);
  ...
  const maxAbs=useMemo(()=>{
    let m=0; cells.forEach(d=>{ if(d){ const k=dk(year,month,d); if(trades[k]) m=Math.max(m,Math.abs(trades[k].totalPnl||0)); } });
    return m||1;
  },[trades,year,month]);

  function createAccount(){
    if(!newAccName.trim()) return;
    const acc={id:uid(),name:newAccName.trim()};
    setStore(s=>({ accounts:[...s.accounts,acc], data:{...s.data,[acc.id]:{days:{}}} }));
    setActiveId(acc.id);
  ...
  function deleteAccount(id){
    ...
    if(activeId===id) setActiveId(accounts.filter(a=>a.id!==id)[0]?.id||null);
  }
  ...
  function saveDayRows(dayKey, rows){
    setStore(s=>{
      const data={...s.data};
      const acc={...(data[activeId]||{days:{}})};
      const days={...acc.days};
      if(rows.length===0) delete days[dayKey]; else days[dayKey]=rows;
      acc.days=days; data[activeId]=acc;
      return {...s, data};
    });
    setFlashKey(dayKey); setTimeout(()=>setFlashKey(null),600);
  }
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0a0a0a",color:"#e0e0e0",fontFamily:"'JetBrains Mono','Fira Code',monospace",padding:"18px 14px",boxSizing:"border-box"}}>
      <style>{`
        ...
        ::-webkit-scrollbar{width:4px;height:4px}::-webkit-scrollbar-track{background:#0a0a0a}::-webkit-scrollbar-thumb{background:#222;border-radius:2px}
      `}</style>
      ...
      {detailDay&&activeId&&(
        <DayDetail
          ...
          initialRows={(accData.days||{})[dk(year,month,detailDay)]||[]}
          onClose={()=>setDetailDay(null)}
          onSave={(rows)=>saveDayRows(dk(year,month,detailDay),rows)}
        />
      )}

      <div style={{maxWidth:800,margin:"0 auto"}}>
        <div style={{marginBottom:16}}>
          <div style={{fontSize:9,color:"#444",letterSpacing:"0.18em",textTransform:"uppercase",marginBottom:2}}>Trading Journal</div>
          <div style={{fontSize:20,fontWeight:700,color:"#e0e0e0"}}>P&amp;L_CALENDAR<span style={{color:"#00ff88",animation:"blink 1.2s step-end infinite"}}>_</span></div>
        </div>
        ...
        <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:16,alignItems:"center"}}>
          {accounts.map(a=>(
            <div key={a.id} style={{display:"flex",alignItems:"center"}}>
              <button onClick={()=>setActiveId(a.id)} style={{background:activeId===a.id?"rgba(99,102,241,0.2)":"#111",border:activeId===a.id?"0.5px solid rgba(99,102,241,0.5)":"0.5px solid #1a1a1a",color:activeId===a.id?"#818cf8":"#444",borderRadius:"6px 0 0 6px",padding:"7px 12px",cursor:"pointer",fontFamily:"inherit",fontSize:11,fontWeight:activeId===a.id?700:400}}>{a.name}</button>
              <button onClick={()=>deleteAccount(a.id)} style={{background:activeId===a.id?"rgba(99,102,241,0.1)":"#111",border:activeId===a.id?"0.5px solid rgba(99,102,241,0.5)":"0.5px solid #1a1a1a",borderLeft:"none",color:"#333",borderRadius:"0 6px 6px 0",padding:"7px 8px",cursor:"pointer",fontFamily:"inherit",fontSize:10,lineHeight:1}}>✕</button>
            </div>
          ))}
          {showNewAcc?(
            <div style={{display:"flex",gap:4,alignItems:"center"}}>
              <input autoFocus placeholder="Account name..." value={newAccName} onChange={e=>setNewAccName(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")createAccount(); if(e.key==="Escape")setShowNewAcc(false);}} style={{background:"#111",border:"0.5px solid #333",borderRadius:6,padding:"7px 10px",color:"#e0e0e0",fontSize:11,fontFamily:"inherit",outline:"none",width:160}}/>
