...
function DayDetail({ dk, account, allRows, onClose, onSaved }) {
  ...
  // Global paste listener — Ctrl+V image into whichever row is selected
  useEffect(() => {
    function onPaste(e) {
      ...
      for (const item of items) {
        if (item.type.startsWith("image/")) {
          ...
          if (!file) return;
          const targetId = pasteTarget || rows[rows.length - 1]?.id;
          if (targetId) { handleImage(targetId, file); setPasteTarget(null); }
          break;
        }
      }
    }
  ...
  useEffect(() => {
    ...
    supabase.from("day_notes").select("*").eq("day_key", dk).eq("account", account).maybeSingle().then(({ data }) => {
      if (data) setNotes({ good: data.good || "", improve: data.improve || "" });
    });
  }, [dk, account]);
  ...
  async function saveAll() {
    ...
    for (const r of valid) {
      const payload = { day_key: r.day_key, account: r.account, pair: r.pair, bias: r.bias, rr: r.rr, outcome: r.outcome, profit: parseFloat(r.profit) || 0, image_url: r.image_url || "", trade_note: r.trade_note || "" };
    ...
    await supabase.from("trades").upsert({ id: `${account}_${dk}`, account, pnl: totalPnl, trade_count: tradeCount, date: dk, wins, losses: tradeCount - wins });
    await supabase.from("day_notes").upsert({ day_key: dk, account, good: notes.good, improve: notes.improve });
    setRows(saved);
    setSaving(false);
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.93)", zIndex:200, display:"flex", alignItems:"flex-start", justifyContent:"center", padding:"16px 12px", overflowY:"auto" }}>
      ...
      <div style={{ background:"#0f0f0f", border:"0.5px solid #222", borderRadius:12, width:"100%", maxWidth:820, padding:18 }}>
            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.id}>
                  ...
                  <td style={{ padding:"0 3px 3px", minWidth:160 }}>
                    <textarea
                      placeholder="Trade notes..."
                      value={r.trade_note || ""}
                      onChange={e => updateRow(r.id, "trade_note", e.target.value)}
                      rows={2}
                      style={{ width:"100%", background:"#141414", border:"0.5px solid #222", borderRadius:4, padding:"5px 7px", color:"#e0e0e0", fontSize:11, fontFamily:"'JetBrains Mono',monospace", resize:"none", outline:"none", lineHeight:1.4, boxSizing:"border-box" }}
                    />
                  </td>
                  <td style={{ padding:"0 3px 3px" }}>
                    ...
                    {r.image_url
                      ? <div style={{ position:"relative", display:"inline-block" }}>
                          ...
                          <button onClick={() => updateRow(r.id,"image_url","")} style={{ position:"absolute", top:-4, right:-4, background:"#ff4444", border:"none", color:"#000", borderRadius:"50%", width:12, height:12, fontSize:8, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", lineHeight:1 }}>✕</button>
                        </div>
                      : <button
                          onClick={() => setPasteTarget(r.id)}
                          title="Click then Ctrl+V to paste screenshot"
                          style={{ background: pasteTarget===r.id ? "rgba(0,255,136,0.15)" : "#1a1a1a", border: pasteTarget===r.id ? "0.5px solid rgba(0,255,136,0.5)" : "0.5px solid #222", color: pasteTarget===r.id ? "#00ff88" : "#444", borderRadius:4, padding:"5px 7px", cursor:"pointer", fontSize:9, fontFamily:"inherit", whiteSpace:"nowrap" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginTop:16 }}>
          {[
            { key:"good", label:"✅ What I did well", placeholder:"e.g. Waited for confirmation, stuck to my plan..." },
            { key:"improve", label:"⚠️ What I could improve", placeholder:"e.g. Entered too early, revenge traded..." },
          ].map(({ key, label, placeholder }) => (
            <div key={key}>
              <div style={{ fontSize:9, color:"#444", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:5 }}>{label}</div>
              <textarea
                placeholder={placeholder}
                value={notes[key]}
                onChange={e => setNotes(prev => ({ ...prev, [key]: e.target.value }))}
                rows={3}
                style={{ width:"100%", background:"#141414", border:"0.5px solid #222", borderRadius:6, padding:"8px 10px", color:"#e0e0e0", fontSize:12, fontFamily:"'JetBrains Mono',monospace", resize:"vertical", outline:"none", lineHeight:1.5, boxSizing:"border-box" }}
              />
            </div>
          ))}
        </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (tData) {
        const map = {};
        tData.forEach(r => { map[r.date] = { totalPnl: r.pnl, tradeCount: r.trade_count, wins: r.wins||0, losses: r.losses||0 }; });
        setTrades(map);
